...
def page_index_main(doc, opt=None):
    ...
    # Jupyter/IPython already run an event loop; asyncio.run() cannot be called from it.
    # Run the async builder in a separate thread with its own loop.
    try:
        asyncio.get_running_loop()
        from concurrent.futures import ThreadPoolExecutor
        with ThreadPoolExecutor(max_workers=1) as pool:
            return pool.submit(asyncio.run, page_index_builder()).result()
    except RuntimeError:
